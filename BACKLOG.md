...
> **最終更新: 2026-07-01**（レトロ化 Phase 1-3 完了：ワールドマップ地域タイル / ゴールド表記統一 / ぼうけんのしょ化 / gold+milestoneトースト / dot_bar / dark=ダンジョン + light=そうげん / DESIGN.md 反映）
...
- Phase 3: voter/status 「ぼうけんのしょ」化 — 相棒＆称号カード (.rpg-window) + 「▶ つよさ」窓 (dot_bar 3本: 記録/連続/ゴールド)
- Phase 3+: 店舗・機種ヘッダーに▶+ font-heading (店舗ヘッダーは .rpg-window 主役化)
- DESIGN.md 反映済み

### レトロ化 Phase 4 候補 (未着手)
- 記録行 (`_machine_vote_row`) の色味最終調整 (レトロ案 D1 のバッジ・数字周りだけ)。ただし記録動線は絶対に触らない
- ランキング上位バッジのドット絵化検討 (現状は 🥇🥈🥉 絵文字)
- カレンダー UI (収支) のドット風装飾
- お気に入りボタンの★をドット絵化
...
ヒーロータグラインを打ち手口調に刷新：「今日のリセットを 読み解け！」「みんなの記録で 設定を 暴け！」「今日の勝負の1台を 記録しよう」「今日の収支は ぼうけんのしょへ」の時間帯4種ローテ。
