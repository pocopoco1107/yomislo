...
RSpec.describe "Promotions", type: :request do
  describe "GET /p/:id (click)" do
    let!(:promo) { create(:promotion, target_url: "https://px.a8.net/track?id=42", clicks_count: 0, active: true) }
    ...
    it "アクティブな案件は clicks_count を加算し外部URLへ302する" do
      ...
      expect(response).to redirect_to("https://px.a8.net/track?id=42")
    ...
    it "allowlist に無い host はカウント加算せずルートにリダイレクト" do
      promo.update_columns(target_url: "https://unknown-domain.example.com/track?id=42")
